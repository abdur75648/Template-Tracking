...
##################### Lukas Kanade Template Tracking With Gaussian Pyramid #####################
...
# Pyramid Lukas 
def pyramid_lukas_kanade(frame,template,rect,num_layer):
    # We'll first downsample at once it & then upsample it back stepwise
    
    rect = (rect*(1/2**num_layer)).astype(int) # Downscaled Template Box
    
    for i in range(num_layer):
        frame = cv2.pyrDown(frame) # Downscaled Frame
        template = cv2.pyrDown(template)
    

    # Initialise P -> W as Identity means parameters P1-P6 as zero
    p = np.zeros(6)
    ...
    for i in range(num_layer):
        ...
        # Upsample the image by 1 level
        frame = cv2.pyrUp(frame)

        # Upsample the template by 1 level
        template = cv2.pyrUp(template)

        # Upsample Template Box by 1 level
        rect = (rect*2).astype(int)


    # One more for bringing it back to original resolution
    p = lukas_kanade(frame,template,rect,p)

    return p


# Template Tracking Using Pyramid Lucas Kanade
def pylka_tt(args):
    num_layer = 1
    ...
    for dirs,subdirs,files in os.walk(args.inp_path+"/img"):
        ...
        for file in tqdm(files):
            if file.endswith('.png') or file.endswith('.jpg') or file.endswith('.jpeg'):
                ...
                if(file_number==1):
                    template = frame # Do not crop it here. Crop at each level of pyramid
                frame = (frame * (np.mean(template)/np.mean(frame)).astype(float) ) # Normalise frame as per template

                p = pyramid_lukas_kanade(frame,template,rect,num_layer)
